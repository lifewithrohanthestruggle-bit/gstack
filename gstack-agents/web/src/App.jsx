...
function QueueView({ onBack, onChat }) {
  ...
  const [activePanel, setActivePanel] = useState('AI Summary')
  const panels = {
    'Patient History': 'Rohan’s first check-in was created on 18 September. No previous skin concerns are saved yet.',
    'AI Summary': 'Mild redness, some texture variation, and elevated oiliness were noticed. No diagnosis has been made.',
    'Doctor Notes': 'Your dermatologist will add notes here during the consultation.',
    'Treatment Plan': 'Your personalised plan will appear here after your doctor reviews your screening.',
    'Follow-up': 'The next suggested check-in is 02 October 2026.',
  }
  const journey = [
    { date: 'SEP 18', label: 'Reaction detected', tone: 'coral' },
    { date: 'SEP 25', label: 'Redness improving', tone: 'sage' },
    { date: 'OCT 02', label: 'Follow-up', tone: 'blue' },
    { date: 'OCT 16', label: 'New scan', tone: 'yellow' },
  ]

  return <div className="page tool-page consultation-page"><PageHeader eyebrow="Care queue" title="Live consultation" description="You’re in line. We’ll keep you informed while a doctor gets ready." onBack={onBack} /><section className="consultation-hero"><div className="live-consultation-label"><i /> LIVE CONSULTATION</div><div className="consultation-hero-grid"><div className="position-block"><span>Your position</span><strong>#03</strong><small>2 patients ahead</small></div><div className="wait-block"><span>Estimated waiting:</span><strong>~15 min</strong><small><i /> Doctor available soon</small></div></div><button className="hero-chat-button" onClick={onChat}><Icon name="sparkle" size={16} /> Chat with AI while waiting <Icon name="arrow" size={15} /></button></section><section className="urgent-strip"><div><strong>Need urgent attention?</strong><span>Tell us if something feels serious or is getting worse.</span></div><button onClick={() => setUrgent((value) => !value)}><Icon name="shield" size={15} /> Safety Help</button></section>{urgent && <div className="urgent-message"><Icon name="help" size={17} /><span>If you have breathing difficulty, eye involvement, severe swelling, or rapidly worsening symptoms, seek urgent medical care now.</span></div>}<section className="patient-consultation"><div className="patient-card-head"><div><span className="patient-label">PATIENT #2048</span><h2>AI SCREENING</h2></div><span className="screening-ready"><i /> Ready for doctor</span></div><div className="patient-divider" /><div className="screening-trail">{['Photos', 'Concern', 'Symptoms', 'Previous products', 'Timeline'].map((item, index) => <div className="trail-item" key={item}><span className="trail-number">{index + 1}</span><strong>{item}</strong>{index < 4 && <Icon name="chevron" size={14} />}</div>)}</div><div className="consultation-heading"><div><span className="patient-label">DOCTOR HANDOFF</span><h2>CONSULTATION</h2></div><button className={`start-chat-button ${chatStarted ? 'started' : ''}`} onClick={() => setChatStarted(true)}>{chatStarted ? <><Icon name="check" size={14} /> Chat started</> : <><Icon name="chat" size={14} /> Start Chat</>}</button></div><div className="consultation-tabs">{Object.keys(panels).map((panel) => <button className={activePanel === panel ? 'active' : ''} key={panel} onClick={() => setActivePanel(panel)}>{panel}</button>)}</div><div className="consultation-panel"><span className="panel-kicker">{activePanel}</span><p>{panels[activePanel]}</p>{activePanel === 'AI Summary' && <div className="panel-tags"><span>Redness</span><span>Texture</span><span>Oiliness</span></div>}</div></section><section className="journey-section"><div className="journey-heading"><div><span className="section-kicker">Your timeline</span><h2>MY SKIN JOURNEY</h2></div><button className="text-button">View all <Icon name="arrow" size={14} /></button></div><div className="journey-list">{journey.map((item, index) => <div className="journey-item" key={item.date}><div className={`journey-marker ${item.tone}`}><span /></div><div className="journey-copy"><strong>{item.date}</strong><span>● {item.label}</span></div>{index < journey.length - 1 && <span className="journey-line" />}</div>)}</div></section></div>
}
